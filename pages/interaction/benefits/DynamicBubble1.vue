<template>
  <LayoutContainer>
    <LayoutSection>
      ...
      <div>설명 : 두줄/한줄 가변 처리</div>
      <div> baseTabWidth : {{ baseTabWidth }}</div>
      <div> clientWidth : {{ tabWidth }}</div>
      ...
      <div ref="dynamicBubble" v-sticky class="dynamicBubble">
        <div class="dynamicBorder">
          <button class="extendBtn" @click="unfold = !unfold">open {{ dynamicDataChunkRowCount }} / {{ unfold }}</button>
...
export default {
  ...
  methods: {
    ...
    chunk(arr) {
      const chunk = arr.length / this.dynamicDataChunkRowCount;
      const temparray = [];
